...
import { discoverConfigFiles, scanPortVars } from "./lib/scan.mts"
...
// Usage: node allocate.mts [dir] [--span N] [--env-file .env] [--dry-run] [--json]
...
const result = await withLock(async () => {
  ...
  reg.envs[absDir] = { updatedAt: new Date().toISOString(), projectName: pname, ports }
  if (!dryRun) saveRegistry(reg)
  return { ports, projectName: pname }
})

if (!dryRun) upsertEnvBlock(envFile, renderBlock(result.projectName, result.ports))
