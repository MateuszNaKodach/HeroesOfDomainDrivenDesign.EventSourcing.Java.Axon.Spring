 ...
 * <p>
 * The only gRPC in this app is the connection to Axon Server (axonserver-connector-java). This registers an
 * OpenTelemetry {@link ClientInterceptor} on that channel via Axon 5's {@link ManagedChannelCustomizer} hook -
 * the framework looks the customizer up with {@code getComponent(ManagedChannelCustomizer.class)} and applies it
 * to the {@code ManagedChannelBuilder}, defaulting to identity only {@code IfNotPresent}, so this Spring bean wins.
 * The interceptor is built from the same {@link OpenTelemetry} SDK bean as the Axon/HTTP/JDBC spans, so gRPC
 * client spans (rpc.system=grpc, rpc.service/rpc.method, status) flow through the existing OTLP pipeline.
 * <p>
 ...
 * (io.opentelemetry.api) are always on the classpath - the former via axonserver-connector-java, the latter via
 * the always-present micrometer/OTLP tracing bridge - so only {@code GrpcTelemetry} needs the reflective path.
 * <p>
 ...
 * command/event/query tracing is already provided by the framework's distributed tracing (axoniq-distributed-messaging).
 *
