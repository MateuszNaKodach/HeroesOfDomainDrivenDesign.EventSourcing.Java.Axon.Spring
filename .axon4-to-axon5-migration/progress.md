...
- **Storage-engine path:** `A — JPA via explicit @Bean EventStorageEngine` (frozen 2026-05-09 during Phase 9; corrected after initial "auto-config wins" attempt was shown to be wrong on this combo). Initial attempt assumed AF5 `JpaEventStoreAutoConfiguration`'s `AggregateBasedJpaEventStorageEngineConfigurationEnhancer` would auto-register `AggregateBasedJpaEventStorageEngine`. Verification against AF5 sources showed that with `axon-server-connector` on classpath (pulled in by `axoniq-spring-boot-starter`), the connector's ServiceLoader-discovered `AxonServerConfigurationEnhancer` runs orders of magnitude earlier (`order = Integer.MIN_VALUE + 10` vs JPA enhancer at `Integer.MAX_VALUE - 600`) and registers `AxonServerEventStorageEngine` (DCB-flat) via `registerIfNotPresent(EventStorageEngine, …, SearchScope.ALL)` BEFORE the JPA enhancer ever runs — even with `axon.axonserver.enabled=false`, because the property only gates Spring `@Bean` methods inside `AxonServerAutoConfiguration`, not the ServiceLoader enhancer. Resolution: declare an explicit Spring `@Bean EventStorageEngine` returning `AggregateBasedJpaEventStorageEngine`. `SpringComponentRegistry.hasComponent(…, SearchScope.ALL)` inspects `BeanFactory.getBeanNamesForType()` in addition to the local `Components` map, so the Spring bean blocks BOTH enhancers' `registerIfNotPresent` calls (connector and JPA). Aggregate-based JPA wins unambiguously, regardless of enhancer ordering.
...
- **Path chosen:** **A — JPA, explicit `@Bean EventStorageEngine`.** Initially attempted "auto-config wins" (no bean), but verified against AF5 sources that this is wrong for projects where `axon-server-connector` is on the classpath via `axoniq-spring-boot-starter` even when `axon.axonserver.enabled=false`: the connector's ServiceLoader-discovered `AxonServerConfigurationEnhancer` (`order = Integer.MIN_VALUE + 10`) runs WAY before `JpaEventStoreAutoConfiguration`'s enhancer (`order ≈ Integer.MAX_VALUE - 600`) and would register `AxonServerEventStorageEngine` (DCB-flat) via `registerIfNotPresent(EventStorageEngine, …, SearchScope.ALL)`. Spring beans of `EventStorageEngine`, however, are visible to `SpringComponentRegistry.hasComponent(…, ALL)` (it inspects `BeanFactory.getBeanNamesForType` in addition to the local `Components` map), so an explicit `@Bean EventStorageEngine` blocks BOTH enhancers' `registerIfNotPresent` calls. Outcome: `AggregateBasedJpaEventStorageEngine` wins unambiguously, regardless of enhancer ordering.
- **Bean added:** [`com.dddheroes.heroesofddd.shared.infrastructure.EventStoreConfiguration#eventStorageEngine`](../src/main/java/com/dddheroes/heroesofddd/shared/infrastructure/EventStoreConfiguration.java) — `new AggregateBasedJpaEventStorageEngine(new JpaTransactionalExecutorProvider(emf), eventConverter, UnaryOperator.identity())`. Defaults preserved (no `batchSize` / `gapTimeout` tuning).
- **Per-recipe Maven profile:** `migration-event-store-EventStoreConfiguration` — includes only the new config class (no test includes; recipe is one-shot, no tests). Verify command: `./mvnw -P migration-event-store-EventStoreConfiguration clean test-compile -DskipTests -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false` → BUILD SUCCESS.
...
- **Configuration class touched:** new — `com.dddheroes.heroesofddd.shared.infrastructure.EventStoreConfiguration` (added in this fix-up commit). No AF4 bean existed to delete.
...
- **Decisions emitted (per recipe Output schema):**
  - `path: A`
  - `bean-replaced: added EventStoreConfiguration#eventStorageEngine (no AF4 bean existed to delete; explicit @Bean needed because connector's enhancer would otherwise register DCB-flat AxonServerEventStorageEngine first)`
